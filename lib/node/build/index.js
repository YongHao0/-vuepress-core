...
module.exports = class Build extends EventEmitter {
  ...
  async render () {
    ...
    for (let i = 0, max = this.context.pages.length; i < max; i++) {
      const page = this.context.pages[i];
      renderPages.push(page);
      if (i % MaxRenderCount === 0 || i === max - 1) {
        const pathList = await Promise.all(
          renderPages.map(page => this.renderPage(page))
        );
        // console.log('path list', pathList);
        pagePaths = pagePaths.concat(pathList);
        renderPages.length = 0;
      }
    }

    console.log('编译文件总耗时:', +new Date() - startTime);
